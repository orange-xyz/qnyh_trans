...
import sys
sys.path.append(r'H:\qnyh\my_code\transformer-lm')
...
def mgpu_train(*xs):
    ...
    for i, xs in enumerate(zip(*xs)):
        ...
        with tf.device(assign_to_gpu(i, "/gpu:0")), tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
            lm_losses = model(*xs, train=True, reuse=do_reuse)
...
def transform_roc(X):
    ...
    for i, x in enumerate(X):
        x12 = [start]+x[:max_len]+[delimiter]+x[:max_len]+[clf_token]
        x13 = [start]+x[:max_len]+[delimiter]+x[:max_len]+[clf_token]
        l12 = len(x12)
        l13 = len(x13)
        xmb[i, 0, :l12, 0] = x12
        xmb[i, 1, :l13, 0] = x13
...
def load_qnyh_data(path=r'H:\qnyh\my_code\qnyh_lstm\data\task_data\\'):
    task_file = r'qnyh_task_data_50k.txt'
    grade_file = r'qnyh_task_data_grades_50k.txt'
    class_file = r'qnyh_task_data_classes_50k.txt'
    def _load(file_name):
        with open(path+file_name, 'r') as f:
            lines = f.readlines()
            lines = [x.replace('\n', '').split() for x in lines]
        return lines
    task_data = _load(task_file)
    grade_data = _load(grade_file)
    class_data = _load(class_file)

    return task_data, grade_data, class_data
...
if __name__ == '__main__':
    ...
    task_data, grade_data, class_data = load_qnyh_data()
    trX, vaX = train_test_split(task_data, test_size=0.2, random_state=2019)
    ...
    lm_losses = mgpu_train(X_train, M_train)
    ...
    eval_mgpu_lm_losses = mgpu_predict(X_train, M_train)
    eval_lm_losses = model(X, M, train=False, reuse=True)
    eval_lm_loss = tf.reduce_sum(eval_lm_losses)
